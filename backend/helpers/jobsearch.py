...
class JobSearch:
    ...
    def search(self):
        ...
        labels = list(({k: v for k, v in sorted(self.wordcount.items(), reverse=True, key=lambda x: x[1]) if len(k) > 2}).keys())
        data = list(({k: v for k, v in sorted(self.wordcount.items(), reverse=True, key=lambda x: x[1]) if len(k) > 2}).values())
